...
def detect(event):
    ...
    with buffer_lock:
        if data_buffer["rgb"] is not None and data_buffer["depth"] is not None:
            ...
            # rgb_edges = cv2.Canny(blurred, 40, 80)
            
            # normalized_depth = depth_image.astype(np.float32)
            # normalized_depth = (normalized_depth - normalized_depth.min()) / (normalized_depth.max() - normalized_depth.min())
            # depth_blurred = cv2.GaussianBlur(normalized_depth, (3,3), 0)
            # depth_edges = cv2.Laplacian(depth_blurred, cv2.CV_32F)
            # depth_edges = np.uint8(np.absolute(depth_edges) * 255 / depth_edges.max())
            ...
            # # Find lines in the image
            # rgb_lines = cv2.HoughLinesP(rgb_edges, 1, np.pi/180, 200, minLineLength=70, maxLineGap=10)
            # if rgb_lines is None:
            #     return
            # rgb_lines = rgb_lines.flatten().reshape(-1, 4)
            # rgb_lines = normalize_lines(rgb_lines)
            # rgb_edges = cv2.cvtColor(rgb_edges, cv2.COLOR_GRAY2BGR)

            # for line in rgb_lines:
            #     p1 = (line[0], line[1])
            #     p2 = (line[2], line[3])
            #     cv2.line(rgb_edges, p1, p2, color=(0, 255, 0), thickness=2)
            ...
            # cv2.imshow("canny", rgb_edges)
            # cv2.imshow("Lap", depth_edges)
            # cv2.waitKey(1)
            end_time = time.time()
